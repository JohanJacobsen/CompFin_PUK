	...
	LPXLOPER12	gridTech)
{
	...
	if (numRows > 5 && !kXlUtils::getInt(gridTech, 5, 0, update, &err)) return kXlUtils::setError(err);
	if (numRows > 6 && !kXlUtils::getInt(gridTech, 6, 0, numPr, &err))	return kXlUtils::setError(err);
	if (numRows > 7 && !kXlUtils::getInt(gridTech, 7, 0, boundary, &err))	return kXlUtils::setError(err);
	if (numRows > 8 && !kXlUtils::getDbl(gridTech, 8, 0, barrier, &err))	return kXlUtils::setError(err);
	if (numRows > 9 && !kXlUtils::getInt(gridTech, 9, 0, bt, &err))	return kXlUtils::setError(err);

	//	run
	double res0;
	kVector<double> s, res, eeb;
	if (!kBlack::fdRunner(s0, r, mu, sigma, expiry, strike, dig > 0, pc, ea, smooth, theta, wind, numStd, numT, numX, update > 0, numPr, boundary, barrier, bt, res0, s, res, eeb, err)) return kXlUtils::setError(err);

	//	size output
	if (boundary > 0)
	{
		numRows = 3 + max(s.size(), eeb.size());
		numCols = 4;
	}
	else
	{
	...
	if (boundary > 0)
	{
		...
		kXlUtils::setStr(2, 2, "Time steps", out);
		kXlUtils::setStr(2, 3, "Early Exercise Boundary", out);
		double timestep = 0.0;
		for (k = 3, i = 0; i < eeb.size(); ++i, ++k)
		{
			kXlUtils::setDbl(k, 2, timestep, out);
			timestep += (double)expiry / numT;
			kXlUtils::setDbl(k, 3, eeb(i), out);
		}
	}
	...
	LPXLOPER12	gridTech)
{
	...
	double res0;
	kVector<double> s, res;
	if (!kBlack::fdRunnerTry(s0, r, mu, sigma, expiry, strike, dig > 0, pc, ea, smooth, theta, wind, numStd, numT, numX, update > 0, numPr, res0, s, res, err)) return kXlUtils::setError(err);
	...
	LPXLOPER12  strikes_in)
{
	...
	int	   smooth = 0;
	numRows = getRows(contract);
	if (numRows > 0 && !kXlUtils::getDbl(contract, 0, 0, max_expiry, &err))	return kXlUtils::setError(err);
	if (numRows > 1 && !kXlUtils::getInt(contract, 1, 0, dig, &err))	return kXlUtils::setError(err);
	...
	if (numRows > 4 && !kXlUtils::getInt(contract, 4, 0, smooth, &err))	return kXlUtils::setError(err);
	...
	if (!kXlUtils::getVector(strikes_in, strikes))
		return kXlUtils::setError("input strikes is not a vector");

	//	run
	kVector<double> s;
	kMatrix<double> resM;
	if (!kBlack::fdfwdRunner(s0, r, mu, sigma, max_expiry, strikes, dig > 0, pc, ea, smooth, theta, wind, numStd, numT, numX, update > 0, numPr, s, resM, err)) return kXlUtils::setError(err);

	//	size output
	numRows = numT + 2;
	numCols = strikes.size() + 1;
	LPXLOPER12 out = kXlUtils::getOper(numRows, numCols);
	...
	//	fill output
	kXlUtils::setStr(0, 0, "Expiries/Strikes", out);
	for (int i = 0; i < numRows; ++i) {
		kXlUtils::setDbl(i + 1, 0, dt * i, out);
	}

	for (int i = 0; i < strikes.size(); ++i) {
		kXlUtils::setDbl(0, i + 1, strikes[i], out);
	}

	for (int k = 1; k < numRows; ++k) {
		for (int i = 1; i < numCols; ++i) {
			kXlUtils::setDbl(k, i, resM(k - 1, i - 1), out);
		}
	}
	//	done
	return out;
	...
	LPXLOPER12	gridTech)
{
	...
	kVector<double> s;
	kMatrix<double> resM;
	if (!kBlack::fdfwdRunner_call(s0, r, mu, sigma, max_expiry, dig > 0, pc, ea, smooth, theta, wind, numStd, numT, numX, update > 0, numPr, s, resM, err)) return kXlUtils::setError(err);
	...
	for (int i = 0; i < numCols; ++i) {
		kXlUtils::setDbl(0, i, s(i), out);
	}

	for (int k = 1; k < numRows; ++k) {
		for (int i = 0; i < numCols; ++i) {
			kXlUtils::setDbl(k, i, resM(k - 1, i), out);
		}
	}
	//	done
	return out;
	...
	LPXLOPER12  expiries_in)
{
	...
	int	   smooth = 0;
	numRows = getRows(contract);
	if (numRows > 0 && !kXlUtils::getInt(contract, 0, 0, dig, &err))	return kXlUtils::setError(err);
	if (numRows > 1 && !kXlUtils::getInt(contract, 1, 0, pc, &err))		return kXlUtils::setError(err);
	if (numRows > 2 && !kXlUtils::getInt(contract, 2, 0, ea, &err))		return kXlUtils::setError(err);
	if (numRows > 3 && !kXlUtils::getInt(contract, 3, 0, smooth, &err))	return kXlUtils::setError(err);
	...
	if (!kXlUtils::getVector(expiries_in, expiries))
		return kXlUtils::setError("input expiries is not a vector");

	//	check strikes and expiries are not empty
	if (strikes.empty() || expiries.empty()) return kXlUtils::setError("strikes or expiries vector is empty");

	//	run
	kVector<double> s;
	kMatrix<double> resM;
	if (!kBlack::fdfwdRunner_new(s0, r, mu, sigma, expiries, strikes, dig > 0, pc, ea, smooth, theta, wind, numStd, numT, numX, update > 0, numPr, s, resM, err)) return kXlUtils::setError(err);

	//	size output
	numRows = expiries.size() + 2;
	numCols = strikes.size() + 1;
	LPXLOPER12 out = kXlUtils::getOper(numRows, numCols);
	...
	//	fill output
	kXlUtils::setStr(0, 0, "Expiries/Strikes", out);
	kXlUtils::setDbl(1, 0, 0.0, out);
	for (int i = 0; i < expiries.size(); ++i) {
		kXlUtils::setDbl(i + 2, 0, expiries[i], out);
	}
...
extern "C" __declspec(dllexport) int xlAutoOpen(void)
{
	...
	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		...
		(LPXLOPER12)TempStr12(L""),
		(LPXLOPER12)TempStr12(L""),
		(LPXLOPER12)TempStr12(L"Solve fd for Black-Scholes model."),
		(LPXLOPER12)TempStr12(L""));

	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		(LPXLOPER12)TempStr12(L"xBlackFdTry"),
		(LPXLOPER12)TempStr12(L"QQQQ"),
		(LPXLOPER12)TempStr12(L"xBlackFdTry"),
		(LPXLOPER12)TempStr12(L"params, contract, gridTech"),
		(LPXLOPER12)TempStr12(L"1"),
	...
	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		(LPXLOPER12)TempStr12(L"xBlackFdFwd"),
		(LPXLOPER12)TempStr12(L"QQQQQ"),
		(LPXLOPER12)TempStr12(L"xBlackFdFwd"),
		...
		(LPXLOPER12)TempStr12(L""),
		(LPXLOPER12)TempStr12(L""),
		(LPXLOPER12)TempStr12(L"Solve fd for black model fwd"),
		(LPXLOPER12)TempStr12(L""));

	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		(LPXLOPER12)TempStr12(L"xBlackFdFwd_call"),
		(LPXLOPER12)TempStr12(L"QQQQ"),
		(LPXLOPER12)TempStr12(L"xBlackFdFwd_call"),
		(LPXLOPER12)TempStr12(L"params, contract, gridTech"),
		(LPXLOPER12)TempStr12(L"1"),
	...
	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		(LPXLOPER12)TempStr12(L"xBlackFdFwd_new"),
		(LPXLOPER12)TempStr12(L"QQQQQQ"),
		(LPXLOPER12)TempStr12(L"xBlackFdFwd_new"),
		(LPXLOPER12)TempStr12(L"params, contract, gridTech, strikes, expiries"),
		(LPXLOPER12)TempStr12(L"1"),
		(LPXLOPER12)TempStr12(L"myOwnCppFunctions"),
		(LPXLOPER12)TempStr12(L""),
		(LPXLOPER12)TempStr12(L""),
